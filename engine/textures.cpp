...
template<typename T> constexpr ImageFormat image_format_of(Array<T> source) {
...
Texture create_texture(const ImageFormat& format, v2u32 dimensions, SamplingFilter filter = Linear, WrapMode wrap = Repeat) {
	GLuint id;
...
Texture create_texture(Array<byte> source, const ImageFormat& format, v2u32 dimensions, SamplingFilter filter = Linear, WrapMode wrap = Repeat) {

	auto texture = create_texture(format, dimensions, filter, wrap);
...
template<typename T> Texture create_texture(Array<T> source, v2u32 dimensions, SamplingFilter filter = Linear, WrapMode wrap = Repeat) {
	GLuint id;
	auto format = ImageFormatFromPixelType<T>;
	assert(dimensions.x * dimensions.y <= source.size());
	return create_texture(Array((byte*)source.data(), source.size_bytes()), format, dimensions, filter, wrap);
}
...
Texture load_texture(const cstr path, SamplingFilter filter = Linear, WrapMode wrap = Repeat) {
	...
	auto texture = create_texture(Array<byte>((byte*)&converted[0], width * height * channels * sizeof(f32)), format, dimensions, filter, wrap);
